...
public class ApostuAnitzaSortu extends JFrame {
	...
	BLFacade facade;
	private double d = 0;
	private double Ddesk = 0;
	private ArrayList<Pronostikoa> l = new ArrayList<Pronostikoa>();
	private double kuotaM = 1;
	private JComboBox<Pronostikoa> comboBox;
	private JTextField textField;
	private JTextField textField_1;
	private JButton btnNewButton;
	private final JButton btnNewButton_1 = new JButton("Apustua Gehitu"); //$NON-NLS-1$ //$NON-NLS-2$
	...
	/*public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					ApostuAnitzaSortu window = new ApostuAnitzaSortu();
	 ...
	 */
	public ApostuAnitzaSortu(JFrame a, Erregistratua e) {
		try {
			initialize(a,e);
		} catch(Exception e1) {
			e1.printStackTrace();
	 ...
	 */
	private void initialize(JFrame aurrekoa, Erregistratua e) throws Exception {
...
//					jCalendar1.setCalendar(calendarAct);
		...
		lblNewLabel_1.setBounds(321, 367, 80, 16);
		...
		textField_1.setBounds(420, 364, 116, 22);
		getContentPane().add(textField_1);
		textField_1.setColumns(10);
		
		btnNewButton = new JButton("Apustua Amaitu"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-1$ //$NON-NLS-2$
		btnNewButton.setBounds(594, 409, 126, 31);
		JFrame a = this;
		btnNewButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent arg0) {
				...
				if(d + Ddesk<=e.getDirua()) {
					int z = facade.zenbakiAnitzHandienaLortu();
					z++;
					facade.apustuAnitzaEgin(z, d + Ddesk*2, e.getNan(),kuotaM, l);
					facade.diruaSartu(e.getDirua()-(d + Ddesk), e.getIz(), e.getPasahitza());
					facade.bonoakEguneratu(e.getNan(), e.bonoKopLortu());
					if(!e.erreplikatuListaLortu().isEmpty()) {
						for(Erregistratua e2: e.erreplikatuListaLortu()) {
							if((d + Ddesk*2)<= e2.getDirua()) {
								z++;
								facade.apustuAnitzaEgin(z, d + Ddesk*2, e2.getNan(),kuotaM, l);
								facade.diruaSartu(e2.getDirua()-(d + Ddesk*2), e2.getIz(), e2.getPasahitza());
							}else {
		...
		btnNewButton_1.setBounds(596, 353, 124, 31);
